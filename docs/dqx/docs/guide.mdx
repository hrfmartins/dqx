...
### Custom Error and Warning Columns

By default, DQX adds `_error` and `_warning` columns to the output dataframe to indicate the quality issues.

It is possible to customize the names of these columns by sending extra configurations to the engine.
